...
X = [ones(size(X,"r"),1) X];
[B_computed,bint_computed,r_computed,rint_computed,stats,fullstats] = scidoe_regress(Y,X);
...
// Check more data
[B,bint,r,rint,stats,fullstats] = scidoe_regress(Y,[ones(X),X]);
Bintexpected = [
  -0.7354667    0.2108205  
   1.0012434    1.0029903  
];
...
[B,bint] = scidoe_regress(Y,[ones(X),X]);
Bintexpected = [
   29.094968   35.647889  
  -0.2217218  -0.1139925  
];
...
[B,bint] = scidoe_regress(gdpch,[ones(unempch),unempch]);
Bexpected = [
0.859
-1.817
];
...
[B,bint] = scidoe_regress(Mass,[ones(Height),Height]);
Bexpected = [
-39.062
61.272
];
